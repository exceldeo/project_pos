...
<script>
  ...
  $(document).ready(function() {
    ...
    $("body").on("keyup", "#tambahkan_kode", function() {
      ...
      $.ajax({
        ...
        success: function(html) {
          ...
          $("#tambahkan_jumlah").val(1).attr("max",html[0].jumlah);
    ...
    $("body").on("click", "#tombol-tambahkan", function() {
      ...
      var harga = $("#tambahkan_harga").val();
      var jumlah = $("#tambahkan_jumlah").val();
      var jumlah_max = $("#tambahkan_jumlah").attr("max");
      ...
      if (id.length == 0) {
        ...
        if (item.length > 0) {
          // update total pembelian dalam array items
          var jumlah_lama = item[0].jumlah;
          var total_lama = item[0].total;
          var total_baru = eval(total_lama) + eval(total);
          var jumlah_baru = eval(jumlah_lama) + eval(jumlah);

          if(jumlah_baru > jumlah_max){
            alert("Jumlah melebihi stok");
            return false;
          }

          item[0].jumlah = jumlah_baru;
          item[0].total = total_baru;
        ...
        for (var i = 0; i < items.length; i++) {
            table_pembelian += "<tr id='tr_" + items[i].id + "'>" +
          "<td> <input type='hidden' name='transaksi_produk[]' value='" + items[i].id + "'> <input type='hidden' name='transaksi_harga[]' value='" + items[i].harga + "'> <input type='hidden' name='transaksi_jumlah[]' value='" + items[i].jumlah + "'> <input type='hidden' name='transaksi_total[]' value='" + items[i].total + "'>" +
          items[i].kode +
          "</td>" +
          "<td>" + items[i].nama + "</td>" +
          "<td align='center'>Rp." + formatNumber(items[i].harga) + ",-</td>" +
          "<td align='center'>" + formatNumber(items[i].jumlah) + "</td>" +
          "<td align='center'>Rp." + formatNumber(items[i].total) + ",-</td>" +
          "<td align='center'> <span class='btn btn-sm btn-danger tombol-hapus-penjualan' total='" + items[i].total + "' jumlah='" + items[i].jumlah + "' harga='" + items[i].harga + "' id='" + items[i].id + "'><i class='fa fa-close'></i> Batal</span></td>" +
          "</tr>";
        }
        $("#table-pembelian tbody").html(table_pembelian);
